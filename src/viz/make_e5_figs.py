...
from us3d.paths import PAPER_FIGS, SESSIONS

ap = argparse.ArgumentParser(description="Build the E5 supervision figures.")
ap.add_argument("--out", type=Path, default=PAPER_FIGS,
                help="directory for the figures (default paper/figs)")
args = ap.parse_args()
...
fig.tight_layout(); fig.savefig(args.out / "e5_matched_region.png", dpi=200)
print("wrote %s" % (args.out / "e5_matched_region.png"))

# ---- Fig 2: flight-143 timeline (ratio trace colored by state + dz) ----
rows = [json.loads(l) for l in
        open(SESSIONS / "section_143" / "supervisor_v3_live.jsonl")]
...
a2.set_ylim(-0.2, 3.4)
fig.tight_layout(); fig.savefig(args.out / "e5_flight143_timeline.png", dpi=200)
print("wrote %s" % (args.out / "e5_flight143_timeline.png"))
